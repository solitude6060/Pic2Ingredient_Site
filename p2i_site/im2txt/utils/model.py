...
def defineRNNmodel(vocab_size, max_len):
	...
	x = add([image_model.output, caption_model.output])
	
	#concatenated = concatenate([image_model.output, caption_model])
	model = Sequential()
	...
	final_model = Model(inputs=[image_model.input, caption_model.input], outputs=model_output)
	final_model.summary()
